...
import sys
sys.path.append('nn/utils')

from viz import gallery
...
def generate_falling_bouncing_ball_dataset(dest,
    ...
    if cifar_background:
        ...
        cifar_train_dataset = datasets.CIFAR10(root='./data', train=True, download=True, transform=None)
        cifar_test_dataset = datasets.CIFAR10(root='./data', train=False, download=True, transform=None)
        x_train, y_train = cifar_train_dataset.data, cifar_train_dataset.targets
        x_test, y_test = cifar_test_dataset.data, cifar_test_dataset.targets
...
def generate_spring_mnist_dataset(dest,
    ...
    import torchvision.datasets as datasets
    mnist_train_dataset = datasets.MNIST(root='./data', train=True, download=True, transform=None)
    mnist_test_dataset = datasets.MNIST(root='./data', train=False, download=True, transform=None)
    x_train, y_train = mnist_train_dataset.data.numpy(), mnist_train_dataset.targets.numpy()
    x_test, y_test = mnist_test_dataset.data.numpy(), mnist_test_dataset.targets.numpy()
